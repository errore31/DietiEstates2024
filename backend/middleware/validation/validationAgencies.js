...
export const validationUpdateAgency = [
    ...
    body("name").optional().trim().notEmpty().withMessage("Il nome non può essere vuoto").isLength({ min: 2, max: 100 }).withMessage("Il nome deve avere tra 2 e 100 caratteri"),
    body("address").optional().trim().notEmpty().withMessage("L'indirizzo non può essere vuoto").isLength({ min: 5, max: 255 }).withMessage("L'indirizzo deve avere tra 5 e 255 caratteri"),
    body("phone").optional().trim().notEmpty().withMessage("Il telefono non può essere vuoto").isLength({ min: 6, max: 20 }).withMessage("Il numero di telefono non è valido").matches(/^[0-9\s\+]+$/).withMessage("Il telefono può contenere solo numeri, spazi e il simbolo +"),
    body("email").optional().trim().notEmpty().withMessage("L'email non può essere vuota").isLength({ max: 100 }).withMessage("L'email non può superare i 100 caratteri").isEmail().withMessage("Indirizzo email non valido").normalizeEmail({ gmail_remove_dots: false }),
];